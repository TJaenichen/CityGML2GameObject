    ...
    public static class Poly2Mesh {
        ...
        public class Polygon {
            ...
            public Vector2 ClosestUV(Vector3 pos) {
                ...
                float bestDSqr = (oriExt[0] - pos).sqrMagnitude;
                for (int i = 0; i < oriExt.Count; i++)
                    for (int t = i + 1; t < oriExt.Count; t++)
                        if (oriExt[i] == oriExt[t])
                            oriExt[t] = new Vector3(0, 0, 0);
                for (int i = 1; i < oriExt.Count; i++)
                {
                    float dsqr = (oriExt[i] - pos).sqrMagnitude;
                    if (dsqr == 0)
                        return outsideUVs[i];
                    if (dsqr < bestDSqr)
                    {
...
//			Debug.Log("Rotated " + originalPos.ToString("F4") + " to " + p.ToString("F4"));
        ...
        public static Mesh CreateMesh(Polygon polygon) {
            ...
            Poly2Tri.Triangulation.Polygon.Polygon poly = new Poly2Tri.Triangulation.Polygon.Polygon(ConvertPoints(polygon.outside, polygon.rotation, codeToPosition));

            // Convert each of the holes
            foreach (List<Vector3> hole in polygon.holes) {
                poly.AddHole(new Poly2Tri.Triangulation.Polygon.Polygon(ConvertPoints(hole, polygon.rotation, codeToPosition)));
            }
